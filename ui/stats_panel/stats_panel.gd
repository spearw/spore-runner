...
func _refresh_player_stats():
	...
	critical_chance_label.text = "Critical Hit Chance: %.0f%%" % (100 * player.get_stat("crit_chance"))
	critical_damage_label.text = "Critical Hit Damage: %.0f%%" % (100 * player.get_stat("crit_damage"))
